...
class BaseFunctions
{
    ...
    public function __construct(){
        ...
        if (isset($_POST['registerUser'])) {
            ...
            if(isset($_POST['ajxregusrname'])){
                $args['ajxregusrname'] = trim(htmlspecialchars($_POST['ajxregusrname']));
            }else{
                $args['ajxregusrname'] = "";
            }
            if(isset($_POST['ajxregusremail'])){
                $args['ajxregusremail'] = trim(htmlspecialchars(strtolower($_POST['ajxregusremail'])));
            }else{
                $args['ajxregusremail'] = "";
            }
            if(isset($_POST['ajxregusrpassword'])){
                $args['ajxregusrpassword'] = trim(htmlspecialchars($_POST['ajxregusrpassword']));
            }else{
                $args['ajxregusrpassword'] = "";
            }
            if(isset($_POST['ajxregusracctc'])){
                $args['ajxregusracctc'] = trim(htmlspecialchars($_POST['ajxregusracctc']));
            }else{
                $args['ajxregusracctc'] = "";
            }
            if(isset($_POST['ajxregusraccnl'])){
                $args['ajxregusraccnl'] = trim(htmlspecialchars($_POST['ajxregusraccnl']));
            }else{
                $args['ajxregusraccnl'] = "";
            }
            if(isset($_POST['ajxregusrsource'])){
                $args['ajxregusrsource'] = trim(htmlspecialchars($_POST['ajxregusrsource']));
            }else{
                $args['ajxregusrsource'] = "";
            }

            $args['ajax'] = true;

            if ($this->registerUser($args)) {
                // will handle the redirect inside the function
        ...
        if ($this->view=="b_acc_login") {
        ...
        }elseif ($this->view=="b_acc_dashboard") {
            if ($this->ID) {
                #code ...
            }else{
                $this->redirect = $this->buildUrl(array('view'=>'b_acc_login'));
            ...
            if ($this->user_is_logged_in) {
                ...
                if (isset($_SESSION['id_user'])) {
                    // code...
                    $this->rep['user'] = $this->getUserById($_SESSION['id_user']);
                    if (!isset($this->rep['user']->ID)) {
                        $this->redirect = $this->buildUrl(array('view'=>"f_404"));
                    }elseif ($this->rep['user']->status==3) {
                        $this->redirect = $this->buildUrl(array('view'=>"b_acc_login"));
                    }elseif ($this->rep['user']->status==1) {
                        // code...
                    }else{
                        $this->redirect = $this->buildUrl(array('view'=>"f_404"));
                    }
    ...
    protected function loginWithPostData( $params=array() ) {
        ...
        $this->rep['ajxrsp'] = array();
        $this->rep['ajxrsp']['success'] = false;
        $this->rep['ajxrsp']['msg'] = "Init.";
        ...
        if (!isset($user->ID)) {
            ...
            $this->rep['ajxrsp']['msg'] = "Emailul sau parola sunt greșite.";
        }elseif (!password_verify($password, $user->password)) {

            if (!$ajax) {
                $_SESSION['msg_errors'][] = "Emailul sau parola sunt greșite...";
            }
            $this->rep['ajxrsp']['msg']="Parola.";
            ...
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['msg'] = 'Contul este șters. Îl poți recupera <a href="'.$this->buildUrl(array('view'=>'b_acc_recover')).'">aici</a> până la data de '.$user->delete_time;
            }
        }elseif ($user->status==1) {
            ...
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['msg'] = 'Pentru a continua trebuie sa confirmati adresa de email accesând linkul din mesajul primit. Dacă nu ați primit mesajul îl puteți cere încă o data de <a href="'.$this->buildUrl(array('view'=>'b_acc_register_confirm_send')).'">aici</a>';
            }
            ...
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['msg'] = 'Adresa de email a fost confirmată, un moderator va activa contul în curând.';
            }
        }elseif ($user->status==3) {
            if (!$ajax) {
                // code...
            }else{
                $this->rep['ajxrsp']['msg'] = "Status 3.";
                $this->rep['ajxrsp']['success']=true;
            }
            $this->user_is_logged_in = true;
            $this->setValues($user);
            $_SESSION['loggedin'] = true;
            ...
            $args['target_table'] = "users";
            $args['id_target'] = $this->ID;
            $args['note'] = "User &ldquo;".$this->username."&rdquo; logged in.".(($ajax)?' (ajax)':'');
            $this->logAction($args);

            return true;
        }else{
            $this->rep['ajxrsp']['msg'] = 'Eroare necunoscută';
        }
        return false;
    }
    ...
    protected function registerUser( $params ) {
        ...
        $this->rep['ajxrsp'] = array('success'=>false, 'msg'=>"Init.");


        $this->rep['ajxrsp']['fields']['name'] = $lastname = isset($params['ajxregusrname'])?$params['ajxregusrname']:"";
        $this->rep['ajxrsp']['fields']['email'] = $email = isset($params['ajxregusremail'])?$params['ajxregusremail']:"";
        $this->rep['ajxrsp']['fields']['password'] = $password = isset($params['ajxregusrpassword'])?$params['ajxregusrpassword']:"";
        $this->rep['ajxrsp']['fields']['acctc'] = $acc_tc = isset($params['ajxregusracctc'])?$params['ajxregusracctc']:"";
        $this->rep['ajxrsp']['fields']['accnl'] = $acc_nl = isset($params['ajxregusraccnl'])?$params['ajxregusraccnl']:"";
        $this->rep['ajxrsp']['fields']['source'] = $source = isset($params['ajxregusrsource'])?$params['ajxregusrsource']:"";
        ...
        if (empty($password)||strlen($password)<8) {
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['errors']['ajxregusrpassword'] = "&ldquo;Parola&rdquo; trebuie să conțină cel puțin 8 caractere.";
            }
            $this->errflag = true;
        }else {
        ...
        if (empty($lastname)) {
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['errors']['ajxregusrname'] = "Vă rugăm să completați câmpul &ldquo;Nume&rdquo;";
            }
            $this->errflag = true;
        }
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['errors']['ajxregusremail'] = "Vă rugăm să completați corect câmpul email";
            }
            $this->errflag = true;
        }else {
            ...
            if (isset($checkEmail->ID)) {
                if (!$ajax) {
                    ...
                    $this->rep['ajxrsp']['errors']['ajxregusremail'] = "Adresa e-mail ".$email." este deja folosită";
                }
        ...
        if ($acc_tc!="da") {
            if (!$ajax) {
                ...
                $this->rep['ajxrsp']['errors']['ajxregusracctc'] = "Vă rugăm să acceptați termenii și condițiile site-ului.";
            }
            $this->errflag = true;
        }
        ...
        if (isset($this->rep['ajxrsp']['errors'])) {
            $this->rep['ajxrsp']['msg'] = implode("<br>", $this->rep['ajxrsp']['errors']);
        }
        ...
        */
        ...
        if ($this->databaseConnection()) {
            ...
            $q->bindValue(":source", $source, PDO::PARAM_STR);
            $q->bindValue(":secretkey", DB_SECRET, PDO::PARAM_STR);
            ...
            if ($r>0) {
                ...
                if (!$this->registerSendConfirmationEmail($userObj)) {
                    if (!$ajax) {
                        ...
                        $this->rep['ajxrsp']['errors']['email_send'] = "A apărut o eroare la trimiterea mesajului email de confirmare.";
                    }
                    // no redirect (maybe) because the email was not sent
                }else{
                    ...
                    if (!$ajax) {
                        ...
                        $this->rep['ajxrsp']['redirect'] = $this->buildUrl(array('view'=>"b_acc_register_success"));
                    }
                }
                ...
                if (!$ajax) {
                    ...
                    $this->rep['ajxrsp']['msg'] = '
                        ...
                        <p>Un email pentru confirmare adresei de email a fost trimis la: "'.$email.'". Urmareste indicatiile din mesaj pentru a finaliza crearea contului.</p>
                        <p>Daca nu ai primit emailul (nici in junk/spam) apasa <a href="'.$this->buildUrl(array('view'=>'b_acc_recover')).'">aici</a> pentru a-l retrimite</p>';
                        $this->rep['ajxrsp']['success'] = true;
                }
            ...
            $this->rep['ajxrsp']['msg'] = 'no databaseConnection';
        }
        return false;
    }
    protected function registerSendConfirmationEmail( $userObj ) {
        $link_activare_cont = $this->buildUrl(array('view'=>'b_acc_register_confirm', 'tve'=>$userObj->token_validare_email));


        $email = $userObj->email_user;
        $subject = PLATFORM_NAME."Confirmare adresa email";
        $name = $userObj->lastname_user;
            ...
            if ($r) {
                ...
                if (USER_REGISTRATION=="auto") {
                    if (USER_AUTO_LOGIN) {
                        ...
                        $this->newRememberMeCookie();
                        $this->redirect = $this->buildUrl(array('view'=>'b_acc_dashboard'));
                        $_SESSION['msg_success'][] = 'Adresa de email a fost confirmată.';
                    } else {
    ...
    protected function accResetPassword( $params ) {
        ...
        if (empty($password_new)||empty($password_repeat)) {
            $this->errflag = true;
            $_SESSION['msg_errors'][] = "Parola trebuie să conțină cel puțin un caracter.";
    ...
    protected function registerLector( $params ) {
    ...
    protected function databaseConnection(){
        if ($this->db_connection != null) {
            $this->rep['ajxrsp']['databaseConnection'] = '3084';
            return true;
        } else {
            $this->rep['ajxrsp']['databaseConnection'] = '3087';
            try {
                $this->rep['ajxrsp']['databaseConnection'] = '3089';
                $this->db_connection = new PDO('mysql:host='. DB_HOST .';dbname='. DB_NAME . ';charset=utf8', DB_USER, DB_PASS);
                return true;
            } catch (PDOException $e) {
                $_SESSION['msg_errors'][] = MESSAGE_DATABASE_ERROR . $e->getMessage();
                $this->rep['ajxrsp']['databaseConnection'] = '3094 '.MESSAGE_DATABASE_ERROR . $e->getMessage();
            }
        }
        return false;
    }
